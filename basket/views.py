    ...
    if size:
        if item_id in list(basket.keys()):
            if size in basket[item_id]['items_by_size'].keys():
                ...
                messages.success(
                    request,
                    f'Updated size {size.upper()}{sweet.name} \
        ...
        if item_id in list(basket.keys()):
            ...
            messages.success(request, f'Updated{sweet.name} \
                quantity to {basket[item_id]}')
        else:
            basket[item_id] = quantity
            messages.success(request, f'Added{sweet.name} \
                to your basket')
    ...
    if size:
        if quantity > 0:
            ...
            messages.success(request, f'Updated size \
                {size.upper()}{sweet.name} quantity \
        ...
        if quantity > 0:
            ...
            basket.pop(item_id)
            messages.success(request, f'Removed {sweet.name} \
                from your bag')
